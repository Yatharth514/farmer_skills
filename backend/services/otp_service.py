...
async def generate_and_store_otp(redis_client,phone:str,role:str):
    ...
    await redis_client.set(f"otp_role:{phone}",role,ex=settings.otp_expiry_seconds)
...
async def verification_of_otp(phone:str,submitted_otp:str,redis_client):
    ...
    if verify_otp(submitted_otp,stored_otp):
        role=await redis_client.get(f"otp_role:{phone}")
        await redis_client.delete(f"otp:{phone}")
        await redis_client.delete(f"otp_attempts:{phone}")
        await redis_client.delete(f"otp_role:{phone}")
        return role
    return None
